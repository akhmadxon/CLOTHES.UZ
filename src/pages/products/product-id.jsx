...
function Index() {
  ...
  const fetchImages = async (id) => {
    try {
      ...
      const imgsDataProps = imgs.map(el => ({
        original: el?.image_url || "",
          ...
          <div className="flex items-center justify-center gap-[60px]">
            ...
            <div>
              ...
              <p className="text-[20px] font-medium text-slate-600 py-1">
                <span className="border-b pb-1 font-semibold">Description :</span>{" "}
                {product?.description}
              </p>
              <p className="text-[20px] text-slate-600">
                Made in:{" "}
                <span className="test-[22px] font-semibold pb-1 border-b">
                  {product?.made_in}
                </span>
              </p>
              <div className="text-[20px] font-medium text-slate-600 flex items-center">
                Color:{" "}
                <p className="ml-3 border-b flex items-center gap-3 pb-1 text-[20px] font-semibold">
              ...
              <div className="flex items-center gap-5 justify-end pt-4">
                ...
                <UploadFileModal dataId={id} />
                <ProductModal dataEdit={dataEdit} />
              </div>
            </div>
          </div>
        </div>
